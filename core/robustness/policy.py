    ...
    def evaluate(
        ...
        *,
        ...
        if protected:
            ...
            # Merely identifying or discussing the standard location is allowed.
        ...
        if _PRIVILEGE_TERMS.search(n):
            ...
            if not verified_authorization:
                return PolicyDecision(Verdict.BLOCK, tuple(categories), "self-asserted role is not authorization", c)
        ...
        # Identity/process diagnostics remain benign unless another capability
        # above raises the risk floor.
        if re.search(r"\b(whoami|\bid\b|groups|ps\b|uname|uptime)\b", n):
            categories.append("benign_system_diagnostic")
            return PolicyDecision(Verdict.ALLOW, tuple(categories), "read-only diagnostic", c)

        # Long or heavily transformed unknown inputs should be reviewed instead
        # of silently allowed.  Simple benign text remains allowed.
